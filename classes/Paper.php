    ...
    public function create()
    {
        ...
        try
        {
                ...
                if (isset($this->file) && $this->file['error'] === UPLOAD_ERR_OK) {
                    ...
                    $fileName = $this->user_id.'_'.time() . '.' . $ext;           
                    ...
                    if (move_uploaded_file($this->file['tmp_name'], $targetPath)) {
                        $this->file = $fileName; // save only the name or relative path
                    } else {
                ...
                $query = "Insert into ".$this->table_name." set 
                            user_id=:user_id, title=:title, type=:type, abstract=:abstract, 
                            co_authors=:co_authors, keywords=:keywords, file=:file";
                ...
                $this->type = htmlspecialchars(strip_tags((string) $this->type));
                $this->abstract = htmlspecialchars(strip_tags((string) $this->abstract));
                $this->co_authors = htmlspecialchars(strip_tags((string) $this->co_authors));
                $this->keywords = htmlspecialchars(strip_tags((string) $this->keywords));
                $this->file = $fileName;
                ...
                $stmt->bindParam(":co_authors", $this->co_authors);
                ...
                if ($stmt->execute())
                {
                    $data = [
                        'status' => 'success',
                        'message' => "The Paper has been successfully created"
                    ];
                ...
                else
                {
                    $data = [
                        'status' => 'fail',
                        'message' => "An error occurred creating the Paper"
                    ];
    ...
    public function update()
    {
        ...
        try
        {
                ...
                $query = "Update ".$this->table_name." set 
                            title=:title, type=:type, abstract=:abstract, 
                            co_authors=:co_authors, keywords=:keywords, file=:file where user_id=:user_id";
